...
| `roles/common/` | NTP/DNS/Syslog vendor-neutral vars | vendor template syntax | role defaults + `molecule/` tests |
| `templates/` | per-vendor Jinja2 render (ios/, eos/) | business logic, facts | render(input intended YAML) -> config text, idempotent |
...
UNVERIFIED: IOSv image version and NetBox API version not externally checked
in this run; pinned before implementation (ASM-002, ASM-004).
...
NetBox (SoT), Ansible + Jinja2 (render/deploy), Containerlab + IOSv (lab),
...
Open: image/API versions UNVERIFIED; routing scope explicitly DEFERRED.
